...
from utils_beit import NativeScalerWithGradNormCount as NativeScaler
import utils_beit
...
def main(args, ds_init):
    ...
    utils.load_model_and_may_interpolate(
        args.finetune, model, 'model|module', ''
    )
    ...
    if args.eval:
        from datasets import (
            ...
            CustomDataset,
            create_dataloader,
            get_sentencepiece_model_for_beit3,
        )
        ...
        if args.extract == "image":
                # items.append(
                #     {
                #         "image_path": os.path.join(args.data_path, frame),
                #         "text_segment": None,
                #         "image_id": len(items),
                #     }
                # )
            ...
            with torch.no_grad(), torch.cuda.amp.autocast():
                ...
                for video in tqdm(sorted(os.listdir(args.data_path))):
                    ...
                    for i, frame in tqdm(enumerate(sorted(os.listdir(os.path.join(args.data_path, video))))):
                    ...
                    np.save(os.path.join(args.output_dir, video + ".npy"), np.concatenate(image_feats, axis=0))


                # loop each chunk of items
                # for i in range(0, len(items), batch_size):
                #     chunks = [
                #         get_image(j) for j in range(i, min(i + batch_size, len(items)))
                #     ]
        ...
        if args.extract == "text":
            ...
            with torch.no_grad(), torch.cuda.amp.autocast():
                ...
                language_tokens = torch.LongTensor(language_tokens).unsqueeze(0).to(device)
                ...
                print(language_cls.shape)
                np.save("text.npy", language_cls.cpu().numpy())
